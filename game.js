...
function moveHorse() {
	...
	for (var num = 0; num < horse.length; num++) {
		...
		} else if (positionLeft >= window.innerWidth * 0.14 - 50 - (num*window.innerWidth*0.02) && positionLeft <= window.innerWidth * 0.95 - 50 - (num*window.innerWidth*0.02) && positionTop <= window.innerHeight * 0.98 - 50 - (num*window.innerHeight * 0.04) && positionTop >= window.innerHeight * 0.85 - 50 - (num*window.innerHeight * 0.04)) {
			horse[num].style.left = positionLeft - 1 + 'px';
		...
		} else if (positionLeft >= window.innerWidth * 0.05 && positionLeft < window.innerWidth * 0.30 - 50 - (num*window.innerWidth*0.02) && positionTop <= window.innerHeight * 0.35 - 50 - (num*window.innerHeight * 0.04) && positionTop >= window.innerHeight * 0.05 - 50) {
			horse[num].style.left = positionLeft + 1 + 'px';
			horse[num].className = 'horse runRight';
		}
	}
